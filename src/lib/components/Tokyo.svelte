<script lang="ts">
    ...
    function handleMouseover(event: MouseEvent) {
        ...
        hoveredName = element.getAttribute('name') ?? '';
...
{#if hoveredName}
    <div class="hover-text" style="left: {mouseX}px; top: {mouseY}px;">
        <ConicGradient style="left: {mouseX}px; top: {mouseY}px;" stops={conicStops} legend>{hoveredName}</ConicGradient>
    </div>
    
{/if}

<svg viewBox="0 0 820 440" xmlns="http://www.w3.org/2000/svg" on:mousemove={handleMousemove}>
    <g id="prefectures">
...
path:hover {
    fill: forestgreen; /* Example hover effect */
}
